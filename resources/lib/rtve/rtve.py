...
from resources.lib.rtve import Home
...
class rtve(object):
    ...
    def getJsonDataNextData(self, url):
        ...
        if link:
            ...
            if script_tag:
                xbmc.log("plugin.video.rtve - found script")
    ...
    def listProgrames(self, nomCategoria):
        xbmc.log("plugin.video.rtve - programes per categoria " + nomCategoria)
        lFolderVideos = []
        categories = self.getTotsProgrames()
        for categoria in categories:
            if nomCategoria == categoria['valor']:
                xbmc.log("plugin.video.rtve - Found categoria " + nomCategoria)
    ...
    def listColeccions(self):
        xbmc.log("plugin.video.rtve - listColeccions")
        lFolderVideos = []

        for categoria in self.getTotsProgrames():
            print(categoria)
            nom_categoria = categoria['valor']
            xbmc.log("plugin.video.rtve - Found categoria " + nom_categoria)
    ...
    def listDirecte(self):
        xbmc.log("plugin.video.rtve listDirecte")
    ...
    def getProgramaData(self, programaId):
        apiUrl = ("https://api.RTVE.cat/videos?version=2.0&_format=json&items_pagina=1000&capitol=1|&tipus_contingut=PPD&ordre=capitol&idioma=PU_CATALA&programatv_id={}&perfil=pc"
                  .format(programaId))
    ...
    def getListTemporades(self, programaTitol):
        xbmc.log("plugin.video.rtve llista temporades " + programaTitol)
        lFolderVideos = []
    ...
    def getListVideos(self, url):
        xbmc.log("plugin.video.rtve - get list videos " + str(url))
