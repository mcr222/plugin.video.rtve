...
class UI(object):
    ...
    def run(self, mode, url):
        ...
        if mode == None:
            ...
            if len(lFolder) > 0:
                ...
                xbmc.log("plugin.video.rtve - UI.run() programes - No existeixen elements")
            ...
            if len(lFolder) > 0:
                ...
                xbmc.log("plugin.video.rtve - UI.run() sections - No existeixen elements")
            ...
            if len(lVideos) > 0:
                ...
                xbmc.log("plugin.video.rtve - UI.run() cercar - No s'ha trobat cap video")
            ...
            xbmc.log("plugin.video.rtve - Temporades")
            lFolder = self.tv3.getListTemporades(url[0])
            self.listFolder(lFolder)

        elif mode[0] == 'coleccions':
            xbmc.log("plugin.video.rtve - Coleccions")
    ...
    def playVideo(self,videoId):
        ...
        if str(videoId).lower().startswith("http"):
            xbmc.log("plugin.video.rtve - UI - is stream link")
            # Simple MP4 playback
            xbmc.Player().play(videoId)
            return

        apiJsonUrl = "https://api-media.RTVE.cat/pvideo/media.jsp?media=video&versio=vast&idint={}&profile=pc_RTVE&format=dm".format(
            videoId)
        xbmc.log("plugin.video.rtve - UI - playVideo apijson url" + str(apiJsonUrl))
        streamUrl = ""
        ...
        xbmc.log("plugin.video.rtve - UI - playVideo mpd/mp4 file " + str(streamUrl))
        is_mp4 = streamUrl.lower().endswith('.mp4')

        if is_mp4:
            xbmc.log("plugin.video.rtve - UI - is mp4")
            # Simple MP4 playback
